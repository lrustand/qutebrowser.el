...
of the command in Qutebrowser."
  (seq-doseq (command (qutebrowser-rpc-request "list-commands"))
    (let ((func-name (intern (concat "qutebrowser-cmd-" command))))
      (fset func-name
            (lambda (&rest args)
              (:documentation (format "Run the `:%s' command in Qutebrowser.
This command was autogenerated by `qutebrowser-populate-commands'." command))
              (interactive)
              (qutebrowser-send-commands (format ":%s %s" command
                                                 (string-join args " "))))))))
;;;; Modes
