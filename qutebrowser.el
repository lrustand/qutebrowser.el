...
(defun qutebrowser--list-links ()
  "List webpage links."
  (let* ((js "Array.from(document.querySelectorAll('a'))
                   .map((elem)=> [elem.href, elem.text]);")
         (params `(:js-code ,js :win-id ,qutebrowser-exwm-win-id)))

    (thread-last (qutebrowser-rpc-request :js params)
                 (seq-uniq)
                 (seq-filter (lambda (elem)
                               (let ((link (seq-elt elem 0))
                                     (desc (seq-elt elem 1)))
                                 (not (string-empty-p link)))))
                 (seq-map #'seq--into-list))))
...
;;;###autoload
(defun qutebrowser-goto-link (link)
  "Scroll to a webpage anchor.
Interactively prompts for an anchor name with completion.
ANCHOR is the anchor to go to."
  (interactive (list (completing-read "Select: " (qutebrowser--list-links))))
  (qutebrowser-send-commands (format ":open %s" link)))
